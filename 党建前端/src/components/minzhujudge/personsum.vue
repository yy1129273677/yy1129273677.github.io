<template>
  <div>
    <mt-header fixed class="top" title="个人总结">
...
<script>
  import{submitSummary,uploadBase64}from "../../api/api"
  let arr=[];
    export default {
        data(){
            return{
      ...
      methods:{
        ...
        submit(){
          ...
          data.append('comment_id',"CEBBD1A4FF2147C8B9ED0CEA6AE90BCF");
          submitSummary(data).then(function (res) {
            console.log(res);
            if(res.data.code==1){
              alert(res.data.msg);
              _this.$router.push('/minzhujudge');
            }else{
              alert(res.data.msg)
            }
          })
